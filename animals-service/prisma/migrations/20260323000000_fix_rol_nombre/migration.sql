-- Fix: Update sp_crear_refugio_completo to use "propietario" instead of "Propietario - {refugio_name}"
-- The role nombre field should contain only the role name, not the refuge name
-- The refuge association is already tracked via refugio_id in the roles table
...
BEGIN
    ...
    -- FIX: Store only the role name "propietario", not "Propietario - {refuge_name}"
    -- The refuge association is tracked via refugio_id
